    ...
    async scrape() {
        ...
        const travellerPromise = async ({ page, data: link }) => {
            ...
            const getBoards = async () => {
                return await page.$$eval('.body > tbody > .tourney', (rows, link) => (rows.map(row => {
                    let board = {
                        contract: '',
                        score: 0,
                        lin: '',
        ...
        const boardsPromise = async ({ page, data: link }) => {
            ...
            dataObj.boards = dataObj.boards.map(board => (0, utils_1.processBoard)(board, board.contract));
            await page.$$eval('table.handrecords > tbody > tr > td.resultcell + td', cells => (cells.map(cell => cell.textContent) || [])
                .filter(text => text.length > 0))
                .then(cells => cells
                .forEach((cell, idx) => {
                if (dataObj.boards[idx])
